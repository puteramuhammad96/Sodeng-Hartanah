...
(function(){
  ...
  const hero = {
    ...
    mount(items){
      ...
      document.querySelectorAll('.hero-slide').forEach(slide => {
        slide.addEventListener('click', ()=> openModal(this.list[this.index]) );
      });

      this.start(); // autoplay 6s
    },
    start(){
      this.stop();
  ...
  function applyFilters(){
    ...
    if(minp){ list = list.filter(x => parsePriceNum(x.price) >= minp); }
    if(maxp){ list = list.filter(x => parsePriceNum(x.price) <= maxp); }
    if(sort==='plh'){ list.sort((a,b)=> parsePriceNum(a.price)-parsePriceNum(b.price)); }
    if(sort==='phl'){ list.sort((a,b)=> parsePriceNum(b.price)-parsePriceNum(a.price)); }
    render(list);
  }
