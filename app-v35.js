...
(function(){
  ...
  function makeCarousel(images){
    const imgs = images.map((u,i)=>`<img src="${fixImageUrl(u)}" class="${i===0?'active':''}" onerror="this.style.display='none'">`).join('');
    return `<div class="carousel">${imgs}${images.length>1?'<button class="prev">‹</button><button class="next">›</button>':''}</div>`;
  }
  ...
  function applyFilters(){
    ...
    let list = ALL.slice();
    if(q) list = list.filter(x => (x.title||'').toLowerCase().includes(q) || (x.location||'').toLowerCase().includes(q));
  ...
  function setupCarousels(){
    document.querySelectorAll('.carousel').forEach(c=>{
      const imgs = c.querySelectorAll('img'); let i=0;
      const show = n => imgs.forEach((im,idx)=> im.classList.toggle('active', idx===n));
      const p = c.querySelector('.prev'), n = c.querySelector('.next');
      if(p) p.addEventListener('click', (e)=>{ e.stopPropagation(); i=(i-1+imgs.length)%imgs.length; show(i); });
      if(n) n.addEventListener('click', (e)=>{ e.stopPropagation(); i=(i+1)%imgs.length; show(i); });
    });
  }
